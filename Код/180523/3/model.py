...
class EspAlgorithm:
    ...
    def _crossover(self, neurons: List[np.ndarray]) -> List[np.ndarray]:
        ...
        for i in range(len(neurons)):
            crossovered_neurons.append(self._crossover_neurons(neurons[i], neurons[i - 1]))
        return crossovered_neurons
...
class NeatAlgorithm:
    def __init__(self,
                 ...
                 x: np.ndarray,
                 y: np.ndarray,
                 number_of_hidden_neurons: int = 1,
                 number_in_subpop: int = 10,
                 mutation_rate: float = 0.1,
                 threshold: float = 0.5,
                 stagnation_threshold: int = 10,
                 stagnation_generations: int = 10):
        self._number_of_hidden_neurons = number_of_hidden_neurons
        self._number_in_subpop = number_in_subpop
        ...
        self._best_loss = 1000000
        self._best_model = Model(input_size, output_size, number_of_hidden_neurons)
        self._best_neurons = self._best_model.get_weights()
        self._unchanged_generations_num = 0
        self._subpopulations = np.random.rand(number_of_hidden_neurons, number_in_subpop, input_size + number_of_hidden_neurons + output_size)
        self._neurons_cumulative_loss = np.zeros((number_of_hidden_neurons, number_in_subpop))
        self._neurons_num_of_trials = np.zeros((number_of_hidden_neurons, number_in_subpop))

    def _run_trials(self, x: np.ndarray, y: np.ndarray):
        for subpopulation in self._subpopulations:
            for neuron_position in range(self._number_of_hidden_neurons):
                for neuron in subpopulation:
                    model = Model(self._input_size, self._output_size, self._number_of_hidden_neurons)
                    model.change_weights(neuron)
                    loss = model.evaluate_model(x, y)
                    self._neurons_cumulative_loss[neuron_position] += loss
                    self._neurons_num_of_trials[neuron_position] += 1

    def _check_stagnation(self):
        if self._unchanged_generations_num > self._stagnation_generations:
            self._number_of_hidden_neurons -= 1
            self._subpopulations = np.delete(self._subpopulations, 0, axis=0)
            self._subpopulations = np.delete(self._subpopulations,
                                             self._input_size + self._number_of_hidden_neurons,
                                             axis=2)
            self._best_model = Model(self._input_size, self._output_size, self._number_of_hidden_neurons)
            self._best_model.change_weights(self._best_neurons)
            self._best_loss = self._best_model.evaluate_model(self._x, self._y)
            self._unchanged_generations_num = 0

    def _recombination(self):
        new_subpopulation = np.zeros((1, self._number_in_subpop, self._input_size + self._number_of_hidden_neurons + self._output_size))
        for i in range(self._number_in_subpop):
            neuron_position = np.argmin(self._neurons_cumulative_loss[:, i])
            new_subpopulation[0][i] = self._subpopulations[neuron_position][i]
        self._subpopulations = np.append(self._subpopulations, new_subpopulation, axis=0)
        self._number_of_hidden_neurons += 1
        position_to_insert = self._input_size + self._number_of_hidden_neurons
        self._subpopulations = \
    ...
    def run(self, goal_loss: float = 0.01):
        history = {'loss': [], 'models': [], 'models_hidden_size': [], 'accuracy': []}
        while self._best_loss > goal_loss:
            try:
                self._neurons_cumulative_loss = np.zeros((self._number_of_hidden_neurons, self._number_in_subpop))
                self._neurons_num_of_trials = np.zeros((self._number_of_hidden_neurons, self._number_in_subpop))
                beginning_best_loss = self._best_loss
    ...
    def _mutation(self, child: np.ndarray):
